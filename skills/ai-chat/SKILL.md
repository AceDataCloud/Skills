...
| `unattended_policy` | object | Pre-authorize tool use via `allowed_skills`, `allowed_mcp_servers`, and optional `expires_at` |
...
- `unattended_policy` does not use a separate `mode` switch — authorization comes from the allow-lists themselves.
