...
# Personal WeChat Service

Use the user's self-hosted **Personal WeChat service** to operate their personal
WeChat account. The service runs on a Windows host with WeChat Desktop logged in
and exposes an HTTP API.

Users get the service credentials by opening
<https://platform.acedata.cloud/services/wechat-bot>, deploying the Personal
WeChat service, then copying the deployment URL and API token from the deployment
details. If the deployment page shows a URL like
`http://host:8000/api/status?token=...`, use `http://host:8000` as the base URL
and the `token` query value as the API token.

Credentials are injected by the `personalwechat` BYOC connector:

- `PERSONALWECHAT_BASE_URL` — Personal WeChat service base URL copied from the platform deployment details.
- `PERSONALWECHAT_API_TOKEN` — Personal WeChat service `API_TOKEN`. Secret — never echo, print, or log it.
...
If `logged_in=false`, tell the user to open the Personal WeChat service UI / RDP
from the platform deployment details and scan the WeChat QR code. If the API
returns 401, ask the user to reconnect the Personal WeChat connector with the
current API token.
...
Use the normal conversations endpoint first; it prefers the local WeChat DB when
ready and falls back to the service's app DB.
...
Read from the service's decrypted WeChat local databases:
...
The Personal WeChat service permits only `SELECT` and `PRAGMA`:
...
- Do not call logout/restart endpoints from the skill unless the user explicitly asks to repair the Personal WeChat service.
- If the service returns 503 for history, run `python3 $WX refresh-history` once, then retry the read.
- If the server is unreachable, ask the user to check the platform deployment status and copy the latest deployment URL.

## Endpoint Mapping

The helper wraps these Personal WeChat service endpoints:
