...
> **Async:** See [async task polling](../_shared/async-tasks.md). Poll via `POST /flux/tasks` with `{"id": "<task_id>"}`.
...
- All generation is async — always set `"callback_url"` to get a `task_id` immediately, then poll `/flux/tasks` with `{"id": "<task_id>"}`
