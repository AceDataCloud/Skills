...
Animate a portrait image to speak/move in sync with an audio track.
...
| `audio_url` (`/kling/talking-photo`) | URL | Audio track to sync the portrait to (required) |
| `prompt` (`/kling/talking-photo`) | string | Optional prompt to guide the talking-photo motion |
| `model` (`/kling/talking-photo`) | `"kling-v1"`, `"kling-v1-6"`, `"kling-v2-master"`, `"kling-v2-1-master"`, `"kling-v2-5-turbo"`, `"kling-v2-6"` | Model (default `kling-v2-1-master`) |
...
- Talking photo (`/kling/talking-photo`) is a separate endpoint; `image_url` and `audio_url` are both required; supports models up to `kling-v2-6`
