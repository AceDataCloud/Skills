...
| `model_name` (`/kling/motion`) | `"kling-v2-6"`, `"kling-v3"` | Motion endpoint model (default `kling-v2-6`) |
| `mode` (`/kling/motion`) | `"std"`, `"pro"` | Motion endpoint quality mode |
| `keep_original_sound` (`/kling/motion`) | `"yes"`, `"no"` | Keep or mute the reference video's original audio (default `yes`) |
| `watermark_info` (`/kling/motion`) | object | Watermark toggle object: `{ "enabled": true|false }` |
| `image_url` (`/kling/motion`) | URL | Source image for motion transfer |
| `video_url` (`/kling/motion`) | URL | Reference motion video |
| `character_orientation` (`/kling/motion`) | `"image"`, `"video"` | Whether orientation is taken from the image or reference video |
| `prompt` (`/kling/motion`) | string | Optional motion guidance text |
| `callback_url` (`/kling/motion`) | string | Async callback URL for motion tasks |
| `async` (`/kling/motion`) | `true`, `false` | Return task immediately and poll via `/kling/tasks` |
