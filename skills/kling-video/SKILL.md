---
name: kling-video
description: Generate AI videos with Kuaishou Kling via AceDataCloud API. Use when creating videos from text or images, extending existing videos, applying motion control, lip-syncing audio/text to video, or animating a portrait photo with audio. Supports text-to-video, image-to-video, extend, motion generation, lip-sync, and talking-photo with multiple models and quality modes.
license: Apache-2.0
metadata:
  author: acedatacloud
  version: "1.0"
...
Animate a portrait photo so it speaks along with an audio clip. Provide a static image and an audio URL.
...
| `async` (`/kling/lip-sync`) | boolean | Return task ID immediately without waiting (default `false`) |
| `image_url` (`/kling/talking-photo`) | URL | Portrait image to animate (required) |
| `audio_url` (`/kling/talking-photo`) | URL | Audio clip the portrait will speak (required) |
| `prompt` (`/kling/talking-photo`) | string | Optional description to guide animation |
| `model` (`/kling/talking-photo`) | `"kling-v1"`, `"kling-v1-6"`, `"kling-v2-master"`, `"kling-v2-1-master"`, `"kling-v2-5-turbo"`, `"kling-v2-6"` | Model to use (default `kling-v2-1-master`) |
| `duration` (`/kling/talking-photo`) | `5`, `10` | Duration in seconds (default `5`) |
| `mode` (`/kling/talking-photo`) | `"std"`, `"pro"` | Quality mode (default `pro`) |
| `async` (`/kling/talking-photo`) | boolean | Return task ID immediately without waiting (default `false`) |
...
- Talking-photo (`/kling/talking-photo`) animates a static portrait image with audio; it requires both `image_url` and `audio_url`; not available for `kling-v3` / `kling-v3-omni`
