---
name: seedance-video
description: Generate AI dance and motion videos with Seedance (ByteDance) via AceDataCloud API. Use when creating videos from text prompts or animating images into motion videos. Supports multiple models with configurable resolution, aspect ratio, duration, reference inputs, and optional audio generation.
license: Apache-2.0
metadata:
  author: acedatacloud
  version: "1.0"
...
| `doubao-seedance-1-5-pro-251215` | Text+Image-to-Video | Latest 1.x model, audio support |
...
| `doubao-seedance-2-0-260128` | Multimodal video generation | Higher-end 2.0 generation |
| `doubao-seedance-2-0-fast-260128` | Multimodal video generation | Faster 2.0 generation |
| `doubao-seedance-2-0-mini-260615` | Multimodal video generation | Lower-cost 2.0 generation |
...
- `reference_audio` — audio is used as a rhythm/style reference
- `reference_video` — video is used as a motion/style reference
...
### 4. Reference Audio or Video

You can add audio or video reference items to guide motion, rhythm, or scene style:
...
POST /seedance/videos
{
  ...
  "content": [
    {"type": "text", "text": "a dancer moving in sync with the beat under neon lights"},
...
| `callback_url` | string | Optional webhook URL for async completion notifications |
| `async` | `true` / `false` | Return immediately with a task ID (`true`) or wait for the job inline (`false`) |
...
- Each content item has a single payload type — use `text`, `image_url`, `audio_url`, or `video_url`, not multiple payload fields in one item
- Reference items use distinct object shapes: `image_url`, `audio_url`, and `video_url` are separate content types
- `first_frame`, `last_frame`, `reference_image`, `reference_audio`, and `reference_video` roles should match the corresponding content type
...
- Resolution options are `480p`, `720p`, `1080p`, and `4k` — there is no 360p or 540p
- `service_tier` values are `"default"` and `"flex"` (not "standard"/"premium")
- Duration range is **2–15 seconds** and frame counts can go up to **361**
