...
| `duration` | `-1` – `15` (integer) | Duration in seconds (`-1` lets the API auto-select) |
...
- Duration range is **-1 to 15 seconds** (integer). `-1` means auto duration; for fixed values, model-specific limits may still apply server-side
