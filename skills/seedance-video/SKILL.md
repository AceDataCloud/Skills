...
Use `omni_reference_task_type: "auto"`, `"reference"`, `"edit"`, or `"extend"`. `reference` requires at least one `reference_image`, `reference_video`, or `reference_audio`. Edit and extend require `reference_video` and `ratio: "adaptive"`; edit also requires `duration: -1`. Optional `output_format` is `mp4` or `mov`.
