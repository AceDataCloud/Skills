...
| `duration` | integer `-1` – `15` | Duration in seconds. Current OpenAPI allows `-1`, but does not define its semantic meaning; avoid `-1` and use explicit positive durations for predictable behavior. |
