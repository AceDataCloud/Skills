...
| `duration` | integer `-1` – `15` | Duration in seconds. Current OpenAPI allows `-1`, but does not define its semantic meaning. |
...
- Duration uses the current OpenAPI schema range **-1–15**; the semantic meaning of `-1` is not documented in the API spec
