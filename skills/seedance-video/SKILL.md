...
| `duration` | integer `-1` – `15` | Duration in seconds. Current OpenAPI schema permits `-1` to `15`; model-specific minimums may be higher. `-1` behavior is unspecified, so use explicit positive durations for predictable behavior. |
