...
> **Async:** See [async task polling](../_shared/async-tasks.md). Poll via `POST /seedance/tasks` with `{"id": "<task_id>", "action": "retrieve"}`.
...
| `doubao-seedance-2-0-260128` | Text+Image-to-Video | Latest 2.0 generation model |
| `doubao-seedance-2-0-fast-260128` | Text+Image-to-Video | Faster 2.0 generation model |
...
- Task polling uses `id` / `ids` plus `action: "retrieve"` / `"retrieve_batch"` in the `/seedance/tasks` request body, not `task_id`
