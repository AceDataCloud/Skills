...
> **Async:** See [async task polling](../_shared/async-tasks.md). Poll via `POST /nano-banana/tasks` with `{"action":"retrieve","id":"..."}` or batch-query with `{"action":"retrieve_batch","ids":["..."]}`.
## Models
...
| `async` | boolean | Optional async flag for provider-side async handling |
...
- `nano-banana-2-lite` supports `1K` resolution only
- `:official` variants (e.g., `nano-banana:official`) route through the official channel
- Task polling supports `action: "retrieve"` with `id`, or `action: "retrieve_batch"` with `ids`
- `403 forbidden` means the request/result was blocked by provider safety checks for that call
