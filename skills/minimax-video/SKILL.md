...
> **Setup:** See [authentication](../_shared/authentication.md). Creation waits for the completed `task` by default. Set `async: true` or provide `callback_url` to return immediately, then use [async task polling](../_shared/async-tasks.md) with `POST /minimax/tasks`.
...
| `async` | boolean; return a task ID immediately when `true` | `false` |
...
The default synchronous response is `{"task": {...}}`. With `async: true` or
`callback_url`, the response is `{"task_id": "...", "trace_id": "..."}`; save
the task ID for polling or callback recovery.
