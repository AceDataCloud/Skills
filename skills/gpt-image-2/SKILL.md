...
(`size` defaults to `"auto"` on both endpoints. Omit `size` or use `"auto"` to let the model
pick — for `edits` this **retains the reference image's aspect ratio** (no reference image on
`generations`, so `auto` there picks based on the prompt). Custom sizes must have both sides a
