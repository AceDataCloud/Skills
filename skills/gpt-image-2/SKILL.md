...
OpenAI `gpt-image-2` through AceDataCloud. Two endpoints, both **synchronous by default** (return image url(s) directly). Its standout is **editing**: feed real images (logos, QR codes, product shots, screenshots) and it composites/restyles them faithfully — great for on-brand video assets and character consistency.
...
Synchronous response (both endpoints): `{"data":[{"url":"https://...png"}]}` → download `data[0].url`.
Async requests return `{"task_id":"..."}` instead; poll that ID as described below.
