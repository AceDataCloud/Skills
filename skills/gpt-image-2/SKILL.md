...
Omitting `size` is the same as passing `"auto"`: edits first honor explicit size intent in
the prompt (pixels, aspect ratio, orientation, resolution tier, or named canvas), then fall
back to the first reference image size when no intent is detected. Custom sizes must have
both sides a multiple of 16, each side ≤ 3840, total pixels between 655,360 and 8,294,400,
and an aspect ratio ≤ 3:1 — otherwise 400.
