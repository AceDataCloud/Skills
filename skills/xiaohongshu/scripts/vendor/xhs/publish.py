...
def bind_products(page: Page, products: list[str]) -> None:
    """Bind the first matching product for each keyword.
        ...
        selected = page.evaluate(
            ...
            (async () => {{
                ...
                for (let i = 0; i < 50; i++) {{
                    ...
                    if (card && (!loading || getComputedStyle(loading).display === 'none')) {{
                        const checkbox = card.querySelector('.d-checkbox');
                        if (!checkbox) return false;
                        const checked = checkbox.querySelector(
    ...
    if not saved:
        raise PublishError("未找到商品弹窗保存按钮")
    if failed:
        raise PublishError(f"部分商品未找到: {failed}")
