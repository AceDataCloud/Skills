...
APPROVAL_PUBLIC_KEY_PATH = Path(
    os.environ.get(
        "XIAOHONGSHU_APPROVAL_PUBLIC_KEY_PATH",
        "/etc/sandbox-auth/timeout-public.pem",
    )
)
EXECUTION_GRANT_ENV = "XIAOHONGSHU_EXECUTION_GRANT"
...
def _approval_digest(command: str, canonical_payload: dict[str, Any]) -> str:
...
def _decode_approval_part(value: str) -> bytes:
...
def _consume_approval_receipt(digest: str, token: str) -> None:
    if not token:
        raise RuntimeError("confirmed write requires --approval-token from the dry-run")
    try:
        encoded, encoded_signature = token.split(".", 1)
        signature = _decode_approval_part(encoded_signature)
        key = load_pem_public_key(APPROVAL_PUBLIC_KEY_PATH.read_bytes())
        key.verify(signature, encoded.encode())
        receipt = json.loads(_decode_approval_part(encoded))
    except (
        InvalidSignature,
        binascii.Error,
        ValueError,
        ...
        raise RuntimeError("approval token is malformed") from error
    if receipt.get("purpose") != "xiaohongshu-approval-v1":
        raise RuntimeError("approval token purpose is invalid")
    if str(receipt.get("digest", "")) != digest:
        raise RuntimeError("write payload or media changed after approval")
    session_id = os.environ.get("AICHAT_SANDBOX_SESSION_ID", "")
    if not session_id or receipt.get("session_id") != session_id:
        raise RuntimeError("approval token belongs to another sandbox session")
    if int(receipt.get("expires_at", 0)) < int(time.time()):
        raise RuntimeError("approval token expired; run a fresh dry-run")
    if not receipt.get("nonce"):
        raise RuntimeError("approval token is malformed")


def _verify_execution_grant(digest: str, approval_token: str) -> None:
    grant = os.environ.get(EXECUTION_GRANT_ENV, "")
    if not grant:
        raise RuntimeError("confirmed write has no worker execution grant")
    try:
        encoded, encoded_signature = grant.split(".", 1)
        signature = _decode_approval_part(encoded_signature)
        key = load_pem_public_key(APPROVAL_PUBLIC_KEY_PATH.read_bytes())
        key.verify(signature, encoded.encode())
        claim = json.loads(_decode_approval_part(encoded))
    except (
        InvalidSignature,
        binascii.Error,
        ValueError,
        ...
        raise RuntimeError("worker execution grant is malformed") from error
    expected_token_hash = hashlib.sha256(approval_token.encode()).hexdigest()
    session_id = os.environ.get("AICHAT_SANDBOX_SESSION_ID", "")
    if (
        claim.get("purpose") != "xiaohongshu-execution-v1"
        or claim.get("digest") != digest
        or claim.get("session_id") != session_id
        or claim.get("approval_token_sha256") != expected_token_hash
        or int(claim.get("expires_at", 0)) < int(time.time())
        or not claim.get("nonce")
    ):
        raise RuntimeError("worker execution grant does not match this approved write")
...
def _unattended_write_allowed() -> tuple[bool, str]:
    ...
    if active_skill not in {"xiaohongshu", "acedatacloud/xiaohongshu"}:
        return False, "the active unattended skill is not Xiaohongshu"
    try:
        allowed = json.loads(os.environ.get("AICHAT_UNATTENDED_ALLOWED_SKILLS", "[]"))
    except json.JSONDecodeError:
        return False, "the unattended skill allowlist is invalid"
    if not isinstance(allowed, list) or active_skill not in allowed:
        return False, "Xiaohongshu is not pre-authorized for unattended writes"
    expires = os.environ.get("AICHAT_UNATTENDED_EXPIRES_AT")
    if expires:
        try:
            if int(expires) < int(time.time()):
                return False, "the unattended authorization has expired"
        except ValueError:
            return False, "the unattended authorization expiry is invalid"
    ...
    output(
        {
            ...
            "note": "No account change was made. Ask the user to approve this digest before using the worker-signed token and --confirm.",
        }
    )
...
def execute(args: argparse.Namespace) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="acedata-xhs-approved-") as media_dir:
        if args.command in WRITE_COMMANDS:
            ...
            digest = _approval_digest(args.command, canonical_payload)
            if not CONFIRM:
                dry_run(args, payload, digest)
                return {}
            if args.approval_digest != digest:
                raise RuntimeError("write payload or media changed after approval")
            _consume_approval_receipt(digest, args.approval_token)
            _verify_execution_grant(digest, args.approval_token)
...
def _add_write_approval(parser: argparse.ArgumentParser) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_write_approval(publish_video)
    ...
    _add_write_approval(publish_long)
