...
11. After the confirmed preview, run a fresh `browser.find` for `role=button`, `name=发布`. The current declarative creator widget is exposed through that semantic contract even though its page element is custom. Inspect the returned ref and reject `submit-disabled=true`, `disabled`, `aria-disabled=true`, or disabled styling. Click that fresh ref exactly once; do not substitute another visible control when Publish is missing.
12. Follow [reconciliation](./reconciliation.md). Immediate success requires `/publish/success`, `published=true`, or a newly added same-title entry/note ID in note management. A changed draft count, returning to the initial publish page, or click completion alone is not success. Return the canonical note URL when visible.
