...
1. Let aichat2 create or reuse the BrowserSession at `https://www.xiaohongshu.com/explore`, then wait up to 30 seconds in bounded intervals.
...
4. If `browser.snapshot` is unavailable on the dense page, use `browser.screenshot` for this status check. If neither proves the state, ask the user to inspect the managed browser session.
...
3. The user scans locally. Poll every 500 ms only through bounded `browser.wait` mode `duration` and fresh observations, with a maximum matching the visible QR expiry (never more than 5 minutes).
