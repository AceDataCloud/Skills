...
4. **Poll task** — `POST /suno/tasks` with `{"id": "<task_id>"}` until status is complete
...
- All generation is **async** — always set `"callback_url"` to get a `task_id` immediately, then poll `/suno/tasks` with `{"id": "<task_id>"}`
