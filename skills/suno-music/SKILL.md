...
| `duration` | integer | Target track length in seconds (10–360) for custom generation. It is a tendency prompt, not a hard constraint |
...
- `duration` is forwarded as you send it — support varies by model and action, and an unsupported combination may ignore it or return an error, so verify with one request before batching. The request `duration` is only a *target*; returned clips may be much shorter than requested, two clips from one request can differ significantly, and identical requests may produce different lengths
