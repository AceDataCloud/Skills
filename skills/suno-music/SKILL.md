...
- All generation is **async** — set `"async": true` (or `"callback_url"`) to get a task id immediately, then poll `/suno/tasks` using `{"id":"<task_id>"}` or `{"ids":[...],"action":"retrieve_batch"}`
