...
Use 1–3 reference images. `prompt_extend` defaults to `true`; `prompt_extend_mode` defaults to `direct`, and `agent` is text-to-image only. Optional controls include `negative_prompt`, `seed` (0–2147483647), and `watermark` (default `false`). `n` supports 1–6. Output size uses `WIDTH*HEIGHT`, with pixel area between 512×512 and 2048×2048 and aspect ratio between 1:8 and 8:1.
