...
curl -X POST https://api.acedata.cloud/qwen-image/images \
  ...
  -d '{"model":"qwen-image-3.0","prompt":"A simple red circle centered on a clean white background.","size":"1024*1024","n":1,"watermark":false}'
```
...
Responses include `data[].image_url`, `usage`, and top-level `cost` fields (`amount`, `currency`, `list_amount`).
